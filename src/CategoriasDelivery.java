...
public class CategoriasDelivery extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CategoriasDelivery frame = new CategoriasDelivery();
	...
	public CategoriasDelivery() {
		...
		btnAgregarCategoria.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				con.conectar();
				boolean existe = con.existeCategoriaDelivery(descripcion);
				con.cerrarConexion();
				...
				if(existe) {
					...
					con.conectar();
					con.guardarCategoriaDelivery(descripcion);
					con.cerrarConexion();
		...
		btnEliminarCategoria.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tabla_categorias.getSelectedRow()<0) {
					...
					if(input == 0) {
						con.conectar();
						con.eliminarCategoriaDelivery(nombre);
						con.cerrarConexion();
						iniciarCategorias();
					}
				}
	...
	public void modificarCategoria() {
		if(tabla_categorias.getSelectedRow()<0) {
			...
			con.conectar();
			con.modificarCategoriaDelivery(codigo,nombre.toUpperCase());
			con.cerrarConexion();
			
			con.conectar();
			con.ejecutarQuery("UPDATE ARTICULOS_DELIVERY set categoria = '"+nombre.toUpperCase()+"' where categoria = '"+categoria_anterior+"'");
			con.cerrarConexion();
